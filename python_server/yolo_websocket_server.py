...
class YoloModel:
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            if file_size_mb < 1.0:
                logger.warning(
                    f"Model file is suspiciously small ({file_size_mb:.2f} MB). This may not be a valid model file.")
            ...
            if hasattr(self.model, 'ckpt_path'):
                ...
                if actual_path != abs_model_path and actual_path != self.model_path:
                    logger.warning(
                        f"WARNING: Model is using {actual_path} instead of {abs_model_path}")

            # Print model information
            if hasattr(self.model, 'model') and hasattr(self.model.model, 'names'):
                logger.info(
                    f"Model architecture: {type(self.model.model).__name__}")
    ...
    def extract_class_names(self) -> None:
        ...
        if hasattr(self.model, 'names'):
            ...
            logger.info(
                f"Extracted {len(self.class_names)} class names: {list(self.class_names.values())}")
        else:
    ...
    def detect_objects(self, image: np.ndarray, conf: float = 0.25) -> Any:
        ...
        results[0].show()
        # Return the first result since we only process one image
        return results[0]
    ...
    def process_results(self, results: Any) -> Dict[str, Any]:
        ...
        if hasattr(results, 'boxes') and len(results.boxes) > 0:
            ...
            for i in range(len(boxes)):
                ...
                logger.info(
                    f"Detection {i}: class={class_name}, confidence={confidence:.3f}, bbox=[{int(x1)}, {int(y1)}, {int(x2)}, {int(y2)}]")
    ...
    async def process_image(self, image_data: str, conf: float = 0.25) -> Dict[str, Any]:
        ...
        try:
            ...
            if image is None:
                ...
                try:
                    ...
                    if len(nparr) >= y_size:
                        ...
                        logger.info(
                            f"Created grayscale image from Y plane: {dim}x{dim}")
                except Exception as e:
                    ...
                    logger.warning(
                        f"Creating dummy test image of size {dummy_width}x{dummy_height}")
                    image = np.zeros(
                        (dummy_height, dummy_width, 3), dtype=np.uint8)
            ...
            sample = image_data[:100] + \
                "..." if len(image_data) > 100 else image_data
            ...
            async for message in websocket:
                try:
                    ...
                    if data["type"] == "image":
                        ...
                        logger.info(
                            f"Received image: format={image_format}, dimensions={image_width}x{image_height}")
                        ...
                        try:
                            ...
                            detection_results["processing_time_ms"] = round(
                                processing_time, 2)
                        ...
                        if "model_path" in data:
                            try:
                                self.yolo_model = YoloModel(
                                    data["model_path"], self.yolo_model.device)
